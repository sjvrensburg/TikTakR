...
test_that("initialise works given different types of objective functions",
    {
        ...
        # function.
        f_1 <- function(x) (x[1])^2 + (x[2] - 1)^2 + x[1] *
            x[2]
        # 2. Separate objective and gradient
        # functions.
        g <- function(x) c(2 * x[1] + x[2], 2 * (x[2] -
            1))
...
test_that("factory_test with initialise work for constraints functions",
    {
        ...
        ineq_2 <- function(x) list(constraints = ineq_1(x),
            jacobian = ineq_jac(x))
        ...
        eq_1 <- function(x) x[1]
        eq_jac <- function(x) 1
        eq_2 <- function(x) list(constraints = eq_1(x),
            jacobian = eq_jac(x))
        ...
        res_3 <- initialise(n, N, c(0, -2), c(0, 2), f_1,
            test_3)
        res_4 <- initialise(n, N, c(0, -2), c(0, 2), f_1,
            test_4)
        expect_identical(res_3, res_4)

        res_5 <- initialise(n, N, c(0, -2), c(0, 2), f_1,
            test_5)
        res_6 <- initialise(n, N, c(0, -2), c(0, 2), f_1,
            test_6)
        expect_identical(res_5, res_6)

        res_7 <- initialise(n, N, c(0, -2), c(0, 2), f_1,
            test_7)
        res_8 <- initialise(n, N, c(0, -2), c(0, 2), f_1,
            test_8)
